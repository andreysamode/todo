...
export default function App() {
  ...
  const dbRead = async () => {
    ...
    setStatus({ type: "info", message: <div className="i-mdi-check size-5"/> });
  }
  useEffect(() => {
    dbRead();
  }, [])
  ...
  return (
    <main className="font-sans max-w-md mx-auto [&_button]:cursor-pointer">
      ...
      <ul className="pl-0 flex flex-col list-none font-size-5 relative my-9 color-black">
        { items.map((item, index) => <li key={index} className={
          ...
          <hr className={"-left-3 -right-3 absolute my-2 mt-1 b-0 h-1 bg-gradient-to-r from-black from-50% to-transparent to-50% bg-[length:200%] transition-300 transition-all pointer-events-none"+
          ...
          <div className='actions'>
            <button className="i-mdi-chevron-up" onClick={() => handleMoveUp(item.id)} />
            <button className="i-mdi-chevron-down" onClick={() => handleMoveDown(item.id)} />
